...
# e2phokz.rb: script which copies and optionally compresses ext2/ext3 filesystem
# it only copies used block, instead of free blocks it writes zeros
#
# usage:
# e2phokz.rb block_device_or_image_file output_file [stomp_channel_name]
#
...
def parse_args
  ...
  if ARGV.length < 2 or ARGV.length > 3
    puts File.open($0).read.split("\n").collect{|l| l.gsub('# ','') unless l.match(/^# /).nil?}.compact.join("\n")
    exit
  end
  if ARGV.length == 3
    @channel_name = ARGV.last
    connect_stomp
  end

  #test for presence of in file || device
  begin
    warn "Warning: input file #{ARGV.first} is not a block device" unless File::Stat.new(ARGV.first).blockdev?
    @file=File.open(ARGV.first,'rb')
  rescue
    warn "Error: infile #{ARGV.first} cannot be open." && exit
  end

  #test for presence of out file
  begin
    if File::Stat.new(ARGV[1]).blockdev?
      warn "Error: I will not overwrite block device #{ARGV[1]}"
    else
      warn "Error: I will not overwrite output file #{ARGV[1]}"
    end
    exit
  rescue
    warn "OK"
  ...
  if ARGV[1]=='-'
    ...
    begin
      if ARGV[1].split('.').last == 'gz'
        #compression required
        @gzip=Zlib::GzipWriter.open(ARGV[1])
      else
        @gzip=File.open(ARGV[1],'wb')
      end
    rescue
      warn "Error: cannot open output file #{ARGV[1]} for writing."
    end
  end
...
def parse_dumpe2fs
  begin
    dump = File.popen("dumpe2fs #{ARGV.first}",'r').read.split("\n")
  rescue
